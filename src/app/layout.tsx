    ...
    <html lang="en" className={`${poppins.variable} ${rubik.variable}`}>
      <head>
        ...
        {/* MegaTag optimizer — config must run before the optimizer script.
            The optimizer loads the GTM container via gtmId, so no separate
            manual GTM loader is used (the <noscript> below is the JS-disabled
            fallback only and never runs alongside the optimizer). */}
        <Script id="mega-tag-config" strategy="afterInteractive">
          {`window.MEGA_TAG_CONFIG={siteKey:"${MEGA_SITE_KEY}",siteId:"${MEGA_SITE_ID}",pixelId:"${MEGA_PIXEL_ID}",gtmId:"${GTM_ID}"};window.API_ENDPOINT="https://optimizer.gomega.ai";window.TRACKING_API_ENDPOINT="https://events-api.gomega.ai";`}
        </Script>
        <Script
          id="optimizer-script"
